...
|-----------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------|
| [`ptrap_de()`](https://camilo-rl.github.io/pTRAPPING/reference/ptrap_de.md)             | IP vs INPUT DE analysis for TRAP-seq (edgeR LRT/QLF or paired t-test)                                         |
| [`ptrap_volcano()`](https://camilo-rl.github.io/pTRAPPING/reference/ptrap_volcano.md)   | Volcano plot from a single [`ptrap_de()`](https://camilo-rl.github.io/pTRAPPING/reference/ptrap_de.md) result |
| [`ptrap_volcano2()`](https://camilo-rl.github.io/pTRAPPING/reference/ptrap_volcano2.md) | Dual scatter comparing two treatment conditions side by side                                                  |
| [`limma_de()`](https://camilo-rl.github.io/pTRAPPING/reference/limma_de.md)             | Multi-group DE for proteomics / metabolomics (limma `treat` or `eBayes`)                                      |
...
overlays both treatment conditions on a single scatter plot. Each axis
shows the log₂ fold-change (IP / INPUT) for one group, and genes are
...
wraps the limma linear modelling framework for **continuous log-scale
data** (label-free proteomics, metabolomics, or RNA-seq after `voom`).
Here we simulate a three-group proteomics experiment — **control**
(`ctrl`), **treatment A** (`trtA`), and **treatment B** (`trtB`), with n
= 6 replicates each.
...
> **Tip — contrast string naming convention:** Inside
> [`limma_de()`](https://camilo-rl.github.io/pTRAPPING/reference/limma_de.md),
> the design matrix is built as `model.matrix(~ 0 + group)` (pairwise)
> or `model.matrix(~ group)` (reference), where the variable is always
...
[`treat()`](https://rdrr.io/pkg/limma/man/ebayes.html) tests whether
\|logFC\| *exceeds* `lfc_threshold`, providing stronger protection
against proteins with statistical but not biological significance.
Recommended when biological effect sizes are well-defined
(e.g. proteomics):
