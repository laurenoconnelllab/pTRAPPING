...
> estimates are unreliable. The paired t-test avoids this by using only the
...
```{r volcano-nb, fig.alt = "Volcano plot for non-bonded mice"}
ptrap_volcano(
  res_nb,
...
```{r volcano-pb, fig.alt = "Customised volcano plot for pair-bonded mice"}
ptrap_volcano(
  res_pb,
  lfc_threshold = 1,
  fdr_threshold = 0.05,
...
```{r volcano2-default, fig.alt = "Dual scatter comparing nb and pb treatments"}
ptrap_volcano2(
  res_nb, res_pb,
...
```{r volcano2-custom, fig.alt = "Customised dual scatter plot"}
ptrap_volcano2(
  res_nb, res_pb,
  lfc_threshold = 1,
  fdr_threshold = 0.05,
...
**control** (`ctrl`), **treatment A** (`trtA`), and **treatment B** (`trtB`),
with n = 6 replicates each.
...
> **Tip — contrast string naming convention:**
> Inside `limma_de()`, the design matrix is built as
> `model.matrix(~ 0 + group)` (pairwise) or `model.matrix(~ group)`
> (reference), where the variable is always called `group`. Coefficient names
> therefore follow the pattern `group<level>` (e.g. `groupctrl`, `grouptrtA`).
> Pass contrast strings using these names.
...
`treat()` tests whether |logFC| *exceeds* `lfc_threshold`, providing stronger
protection against proteins with statistical but not biological significance.
Recommended when biological effect sizes are well-defined (e.g. proteomics):
...
When all comparisons are against a single baseline, `model_type = "reference"`
is a simpler alternative — the first factor level becomes the reference and
other coefficients are differences from it:
...
```{r explicit-sample-df, eval = FALSE}
